...
class HaarSplitter(nn.Module):
    ...
    def _haar_forward(x):
        ...
        bot_left = x[..., 1::2, 0::2]
        bot_right = x[..., 1::2, 1::2]
        ll = (top_left + top_right + bot_left + bot_right) / 2
        lh = (top_left + top_right - bot_left - bot_right) / 2
        hl = (top_left - top_right + bot_left - bot_right) / 2
        hh = (top_left - top_right - bot_left + bot_right) / 2
        return ll, lh, hl, hh
    ...
    def _haar_inverse(ll, lh, hl, hh):
        ...
        out = torch.zeros(
            *ll.shape[:-2], h * 2, w * 2, device=ll.device, dtype=ll.dtype
        )
        tl = (ll + lh + hl + hh) / 2
        tr = (ll + lh - hl - hh) / 2
        bl = (ll - lh + hl - hh) / 2
    ...
    def forward(self, cond):
        ...
        ll1, lh1, hl1, hh1 = self._haar_forward(cond)

        # 2. Level-2 decomposition on LL1
        ll2, lh2, hl2, hh2 = self._haar_forward(ll1)

        # 3. Low reconstruction (keep only LL2)
        ll1_low = self._haar_inverse(ll2, torch.zeros_like(lh2), torch.zeros_like(hl2), torch.zeros_like(hh2))
        x_rec_low = self._haar_inverse(ll1_low, torch.zeros_like(lh1), torch.zeros_like(hl1), torch.zeros_like(hh1))

        # 4. High reconstruction (keep level-1 details + level-2 details)
        ll1_high = self._haar_inverse(torch.zeros_like(ll2), lh2, hl2, hh2)
        x_rec_high = self._haar_inverse(ll1_high, lh1, hl1, hh1)

        return cond, x_rec_low, x_rec_high
...
class JiTBlock(nn.Module):
    ...
    def forward(self, x, c, cond, low_cond, high_cond, feat_rope=None, shared_cond_w=None, shared_low_cond_w=None, shared_high_cond_w=None):
        ...
        w_c = shared_cond_w if self.cond_mode == 'shared' else (self.cond_w if self.cond_mode in ['learnable', 'learnable_0'] else 1.0)
        w_lc = shared_low_cond_w if self.low_cond_mode == 'shared' else (self.low_cond_w if self.low_cond_mode in ['learnable', 'learnable_0'] else 1.0)
        w_hc = shared_high_cond_w if self.high_cond_mode == 'shared' else (self.high_cond_w if self.high_cond_mode in ['learnable', 'learnable_0'] else 1.0)

        x = x + w_c * cond + w_lc * low_cond + w_hc * high_cond

        x = x + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm3(x), shift_mlp, scale_mlp))
        return x


class JiT_ParaCondWave(nn.Module):
    """
    ...
    def forward(self, x, t, cond=None):
        ...
        if cond is None and self.cond_channels > 0:
            cond = x.new_zeros(
                (x.size(0), self.cond_channels, x.size(2), x.size(3)))
