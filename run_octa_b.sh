...
uv run torchrun --nproc_per_node=1 --nnodes=1 --node_rank=0 \
src/main_jit.py \
--model JiT-B/16 \
--proj_dropout 0.0 \
...
--epochs 50000 --warmup_epochs 100 \
--data_path data/OCTA500_6M/ --dataset OCTA500_6M \
--img_channel 1 --mask_channel 1 \
--online_eval

uv run python alert.py --repo jit --message "OCTA segmentation 50,000 epochs training completed"
# GPU selection (default: 0,1)
...
uv run python src/inference_jit.py --model JiT-B/16 --checkpoint checkpoint-last.pth \
--data_path data/OCTA500_6M/test --output_dir outputs \
--batch_size 4 --num_workers 0 --img_size 256 --img_channel 1 --mask_channel 1 \
--patch_size 256 --stride 128 --metrics --device cuda
